...
class ManageOptionsPanel( ManagePanel ):
    ...
    class _TagsPanel( wx.Panel ):
        ...
        def __init__( self, parent, new_options ):
            ...
            self._suggested_favourites = ClientGUICommon.ListBoxTagsStringsAddRemove( self )
            self._suggested_favourites.SetMinSize( ( 200, 300 ) )
            
            self._suggested_favourites_input = wx.TextCtrl( self, style = wx.TE_PROCESS_ENTER )
            self._suggested_favourites_input.Bind( wx.EVT_TEXT_ENTER, self.EventEnterSuggestedTagFavourite )
            ...
            vbox.AddF( gridbox, CC.FLAGS_EXPAND_SIZER_BOTH_WAYS )
            vbox.AddF( wx.StaticText( self, label = 'Suggested tags - favourites (prototype):' ), CC.FLAGS_MIXED )
            vbox.AddF( self._suggested_favourites, CC.FLAGS_MIXED )
            vbox.AddF( self._suggested_favourites_input, CC.FLAGS_MIXED )
            
            self.SetSizer( vbox )
            
        
        def EventEnterSuggestedTagFavourite( self, event ):
            
            tag = self._suggested_favourites_input.GetValue()
            
            if tag != '':
                
                self._suggested_favourites.AddTags( ( tag, ) )
                
                self._suggested_favourites_input.SetValue( '' )
    ...
    class _Panel( wx.Panel ):
        ...
        def __init__( self, parent, file_service_key, tag_service_key, media, immediate_commit ):
            ...
            if len( favourites ) > 0:
                ...
                suggested_tags.SetMinSize( ( 120, -1 ) )
